...
def test_enregistrer_exit_partielle(tmp_trades_db, monkeypatch):
    db_file = tmp_trades_db
    ...
    conn = sqlite3.connect(db_file)
    rows = conn.execute("SELECT action, prix FROM trades_auto").fetchall()
    conn.close()
    assert rows == [("exit_3pct", 1.03), ("exit_7pct", 1.07)]
